<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="出校时间" prop="chuxiaoshijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.chuxiaoshijian" 
                type="datetime"
                :readonly="ro.chuxiaoshijian"
                placeholder="出校时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.chuxiaoshijian" label="出校时间" prop="chuxiaoshijian">
              <el-input v-model="ruleForm.chuxiaoshijian" 
                placeholder="出校时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="出校事由" prop="chuxiaoshiyou">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="出校事由"
                  v-model="ruleForm.chuxiaoshiyou" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.chuxiaoshiyou" label="出校事由" prop="chuxiaoshiyou">
                    <span>{{ruleForm.chuxiaoshiyou}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.leixing = true;
          ...
          if(o=='chuxiaoshijian'){
            this.ruleForm.chuxiaoshijian = obj[o];
	    this.ro.chuxiaoshijian = true;
            continue;
          }
          if(o=='chuxiaoshiyou'){
            this.ruleForm.chuxiaoshiyou = obj[o];
	    this.ro.chuxiaoshiyou = true;
            continue;
          }
          if(o=='jiankangma'){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `chuxiaodengji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.chuxiaodengjiCrossAddOrUpdateFlag = false;
						 this.parent.search();
