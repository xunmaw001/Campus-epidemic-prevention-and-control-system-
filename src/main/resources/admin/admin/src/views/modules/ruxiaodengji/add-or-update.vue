<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="入校时间" prop="ruxiaoshijian">
            <el-date-picker
                value-format="yyyy-MM-dd HH:mm:ss"
                v-model="ruleForm.ruxiaoshijian" 
                type="datetime"
                :readonly="ro.ruxiaoshijian"
                placeholder="入校时间">
            </el-date-picker>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.ruxiaoshijian" label="入校时间" prop="ruxiaoshijian">
              <el-input v-model="ruleForm.ruxiaoshijian" 
                placeholder="入校时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="24">  
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="入校事由" prop="ruxiaoshiyou">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="入校事由"
                  v-model="ruleForm.ruxiaoshiyou" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.ruxiaoshiyou" label="入校事由" prop="ruxiaoshiyou">
                    <span>{{ruleForm.ruxiaoshiyou}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.leixing = true;
          ...
          if(o=='ruxiaoshijian'){
            this.ruleForm.ruxiaoshijian = obj[o];
	    this.ro.ruxiaoshijian = true;
            continue;
          }
          if(o=='ruxiaoshiyou'){
            this.ruleForm.ruxiaoshiyou = obj[o];
	    this.ro.ruxiaoshiyou = true;
            continue;
          }
          if(o=='jiankangma'){
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `ruxiaodengji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.ruxiaodengjiCrossAddOrUpdateFlag = false;
						 this.parent.search();
